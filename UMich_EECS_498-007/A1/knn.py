...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    x_test = x_test.reshape(num_test,-1)
    for i in range(num_train):
        dists[i,] = ((x_train[i].repeat(num_test,1)-x_test)**2).sum(dim = 1)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    sum_train = (x_train_flat**2).sum(dim=1, keepdim=True)  # Shape (num_train, 1)
    sum_test = (x_test_flat**2).sum(dim=1, keepdim=True)  # Shape (num_test, 1)

    mul = x_train_flat @ x_test_flat.t()

    dists = sum_train + sum_test.t() - 2 * mul
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    _,sort_indices =  dists.sort(dim = 0)
    sort_indices = y_train[sort_indices]
    y_pred,_ = sort_indices[:k,:].mode(dim = 0)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return y_pred
...
def knn_cross_validate(
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    x_train_k = [(x_train_folds[:i] + x_train_folds[i+1:], [x_train_folds[i]]) for i in range(num_folds)]
    y_train_k = [(y_train_folds[:i] + y_train_folds[i+1:], [y_train_folds[i]]) for i in range(num_folds)]
    for k in k_choices:
        acct_list = []
        for j in range(num_folds):
            x_train_one_fold = torch.cat(x_train_k[j][0])
            x_test_one_fold = x_train_k[j][1][0]
            y_train_one_fold = torch.cat(y_train_k[j][0])
            y_test_one_fold = y_train_k[j][1][0]
            classifier = KnnClassifier(x_train_one_fold, y_train_one_fold)
            
            acct_list.append(classifier.check_accuracy(x_test_one_fold, y_test_one_fold, k=k))
        k_to_accuracies[k] = acct_list
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for k in k_to_accuracies.keys():
        accury_updated = torch.mean(torch.Tensor(k_to_accuracies[k]))
        if  accury_updated > accury:
            best_k = k
            accury = accury_updated
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return best_k
