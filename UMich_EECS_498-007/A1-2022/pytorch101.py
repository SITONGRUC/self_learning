...
def count_tensor_elements(x: Tensor) -> int:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    shape_of_tensor = x.shape
    num_elements = 1
    for i in shape_of_tensor:
...
def multiples_of_ten(start: int, stop: int) -> Tensor:
    ...
    start_mut = start//10
    stop_mut = stop//10
    output_t = torch.arange(start_mut,stop_mut+1,dtype = torch.float64 ) 
    x = (output_t[output_t!=0]*10)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return x
...
def slice_indexing_practice(x: Tensor) -> Tuple[Tensor, Tensor, Tensor, Tensor]:
    ...
    even_rows = x[0::2, :]  # Slice from the start, stepping by 2 to get even rows
    even_rows_odd_cols = even_rows[:, 1::2]
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    out = (
...
def reverse_rows(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y  = x[torch.arange(x.shape[0]).tolist()[::-1],] 
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def take_one_elem_per_col(x: Tensor) -> Tensor:
    ...
    Returns:
        ...
        - The first element of y is the second element of the first column of x (2,1)
        - The second element of y is the first element of the second column of x(1,2)
        - The third element of y is the fourth element of the third column of x(4,3)
    """
    y = None
    ##########################################################################
    #                      TODO: Implement this function                     #
...
def make_one_hot(x: List[int]) -> Tensor:
    ...
    index0 = torch.arange(len(x)).tolist()
...
def reshape_practice(x: Tensor) -> Tensor:
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    y = torch.concat((x[:12].reshape(3,4),x[12:].reshape(3,4)),dim=1)
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
    return y
...
def zero_row_min(x: Tensor) -> Tensor:
    ...
    # Replace "pass" statement with your code
    x0 = x
    col_min_vals, col_min_idxs = x0.min(dim = 1)
    x0[torch.arange(x0.shape[0]),col_min_idxs] = 0
    y = x0
    ##########################################################################
    #                            END OF YOUR CODE                            #
    ##########################################################################
...
def batched_matrix_multiply_loop(x: Tensor, y: Tensor) -> Tensor:
    ...
    B = x.shape[0]
    for i in range(B):
        output_list.append(torch.mm(x[i,:,:],y[i,:,:]))
    z = torch.stack(output_list)
